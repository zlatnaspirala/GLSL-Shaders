...
void mainImage(out vec4 outColor, in vec2 fragCoord) {
  ...
  for(int m = 0; m < AA; m++) for(int n = 0; n < AA; n++) {
      ...
      // animation
      float ta2 = 3.14f * (0.5f + 0.5f * cos(iTime * 0.22f + 2.0f));
      //
      float tb2 = 3.14f * (0.5f + 0.5f * cos(1.0f * 0.11f + 2.0f));
      //
      float rb2 = 0.15f * (0.5f + 0.5f * cos(1.0f * 0.21f + 1.0f));

      // sdf(p) and gradient(sdf(p))
      vec3 dg2 = sdgArc(p, vec2(sin(ta2), cos(ta2)), vec2(sin(tb2), cos(tb2)), 0.5f, rb2);
      float d2 = dg2.x;
      vec2 g2 = dg2.yz;
      ...
      // coloring
      vec3 col2 = (d2 > 0.0f) ? vec3(0.9f, 0.6f, 0.3f) : vec3(0.4f, 0.7f, 0.85f);
      col2 *= 1.0f + vec3(0.5f * g, 0.0f);
        // col = vec3(0.5+0.5*g,1.0);
      col2 *= 1.0f - 0.5f * exp(-16.0f * abs(d2));
        // waves
      col2 *= 0.9f + 0.1f * cos(150.0f * d2);
      col2 = mix(col2, vec3(1.0f), 1.0f - smoothstep(0.0f, 0.01f, abs(d2)));
